...
const mocks = vi.hoisted(() => ({ getSession: vi.fn(), headers: vi.fn(), getDb: vi.fn(), createDomainExecutor: vi.fn(), createHouseholdService: vi.fn() }));
vi.mock("server-only", () => ({}));
...
import { getDomainExecutor, getHouseholdService } from "./server";
