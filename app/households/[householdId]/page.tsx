...
export default async function MembersPage({ params }: { params: Promise<{ householdId: string }> }) {
  ...
  try { home = await householdDetails(getDomainExecutor(), householdId); }
    ...
    return <PrivateBoundary><main className="mx-auto max-w-3xl px-6 py-16"><h1 className="text-3xl font-semibold">Household unavailable</h1><p className="my-4 text-muted-foreground">You do not have active access to this household.</p><a href="/households" className="text-primary">Back to your households</a></main></PrivateBoundary>;
  }
  ...
  return <PrivateBoundary><main className="min-h-svh bg-muted/30 px-6 py-10 sm:px-10"><div className="mx-auto max-w-5xl">
    <header className="flex items-center justify-between gap-4"><a href="/households" className="text-sm text-primary">← Switch household</a><SignOutButton /></header>
    ...
    <p className="mt-6"><a className="text-primary underline" href={`/households/${householdId}/invitations`}>Manage invitation links</a></p>
