...
function InvitationConsent({ fragment }: { fragment: string }) {
  ...
  return <div className="mt-6 space-y-5">
    ...
    {(needsAuth || !preview) && <p className="text-sm"><a className="text-primary underline" href={`/sign-in${fragment}`}>Sign in</a> or <a className="text-primary underline" href={`/sign-up${fragment}`}>create an account</a> to accept. Your invitation will be preserved.</p>}
...
export function InvitationManager({ householdId, invitations }: { householdId: string; invitations: { id: string; expiresAt: string }[] }) {
  ...
  async function create() {
    ...
    try {
      ...
      setLink(result.token ? `${window.location.origin}/invitations#${result.token}` : "");
  ...
  async function revoke(id: string) {
    ...
    try {
      ...
      else { setRemoved((items) => [...items, id]); setLink(""); }
    } catch { setError("Unable to connect. Please try again."); }
    finally { setPending(false); }
  }
  return <div className="mt-6 space-y-6"><p className="max-w-xl text-muted-foreground">Anyone with a link can join after signing in and accepting. Links expire in seven days and can be used once. Share only with someone you want in this household.</p>
    ...
    {link && <div className="space-y-2 rounded-xl border bg-background p-5"><label htmlFor="invitation-link" className="font-medium">Your invitation link</label><input id="invitation-link" className="w-full rounded border p-3 text-sm" readOnly value={link} onFocus={(event) => event.currentTarget.select()} /><p className="text-sm text-muted-foreground">Copy this link now. It is shown only once and cannot be recovered after leaving this page.</p></div>}
    {error && <p role="alert">{error}</p>}
